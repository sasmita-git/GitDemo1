...
public class ProctoredAssessmentTest_PNComprehensivePredictor2014 extends BaseTest {
	 ...
	 * @throws Exception
	 */
	@TestAnnotations(testID = "NewATI-934")
	@Test(priority = 1, alwaysRun = true, enabled = true, groups = {"Regression", "Smoke"}, description = "NewATI-934:PN Maternal Newborn 2014")
	public void NewATI934_ProctoredAssessmentTest_PNCP2014() throws Exception {
		log(logger, "NewATI934_ProctoredAssessmentTest_PNCP2014 Start");
		...
		proctorChromePage.loginToApplicationID(proctorDriver,
				PropertiesRepository.getString("id.login.proctor.pncp.2014.username"),
				PropertiesRepository.getString("id.login.proctor.pncp.2014.password"));
		...
		proctorChromePage.enteringProctIdAndPwd(proctorDriver,
				PropertiesRepository.getString("id.proctor.id.value.prod.pncp2014"),
				PropertiesRepository.getString("id.proctor.pwd.value.prod.pncp2014"));//Thread.sleep(10000);Thread.sleep(10000);
		proctorChromePage.waitUntillLoadingImageIsDisappeared(proctorDriver, PropertiesRepository.getString("ATIFaculti.common.page.spinner.loc"));
		...
		proctoredAssessmentPage.enterTextValue(PropertiesRepository.getString("new-atitesting.add.product.id.new.loc"), PropertiesRepository
				.getString("id.proctor.id.value.prod.pncp2014"));
		...
		boolean resultFlag = proctorPage.takeTEASProctorAssessmentContinue_PNCP2014();
		logger.info("Proctor Result --> " + resultFlag);

		log(logger, "Take Proctor Assessment Survey");
		proctorPage.takeTEASProctorAssessmentSurvey_pncp2014();
		...
		// Student End		
		log(logger, "NewATI934_ProctoredAssessmentTest_PNCP2014 End");		
	}	
