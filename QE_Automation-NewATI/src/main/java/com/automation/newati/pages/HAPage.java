...
public class HAPage extends NewATIBasePage {
	...
	public HAPage(WebDriver webDriver) {
		super(webDriver);
	 ...
	 */
	public void waitForElementPresent(String loc,int time) throws Exception
	{
		logger.info("Method: VerifyIfElementPresent Start");
		try {
			WebDriverWait wait = new WebDriverWait(driver, time);
			wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(loc)));
		
		} catch (Exception e) {
			logger.info("There are no enabled HealthAsses Displayed");
			throw new Exception("There are no enabled HealthAsses Displayed");
	}
		logger.info("Method: VerifyIfElementPresent End  ");
	}
	 ...
	 * @throws DriverException
	 */
	public boolean VerifyElementPresent(String loc,String simulationname) throws Exception
	{
		logger.info("Method: VerifyElementPresent Start");
		boolean flag = false;
		try {
			WebElement elepresent=driver.findElement(By.cssSelector(loc));
			if(elepresent.getText().equalsIgnoreCase(simulationname))
			{
				flag=true;
			}
		
		} catch (Exception e) {
			logger.info("There are no such element Displayed");
			throw new Exception("There are no such element Displayed");
	}
		logger.info("Method: VerifyElementPresent End ---> "+flag);
		return flag;
	}
	/**
	 * Verify if HA tutorial is displayed
	 * 
	 * 
	 * @author sasmita.mishra
	 * @throws DriverException
	...
	public boolean VerifyIfHealthAssessDisplayed(String tutorialName) throws Exception {
		logger.info("Method: VerifyIfHealthAssessDisplayed Start");
		boolean flag = false;
		try {
			...
			for (int i = 0; i < moduleList.size(); i++) {
				if (CollectionUtils.isNotEmpty(moduleList) && (moduleList.get(i)
						...
						.getText().equals(PropertiesRepository
								.getString("new-atitesting.apply.tab.simulation.ha.tutor.name")))) {
					flag = true;
					break;
				}
			}
		...
		logger.info("Method: VerifyIfHealthAssessDisplayed End --> " + flag);
		return flag;
	}

	/**
	 * Method used to get module count for HealthAssess.
	 * 
	 * @author sasmita
	...
	public boolean verifyDropdownOptionInHACardback(String val) throws DriverException {
		boolean flag = false;
		try {
			WebElement dropdown = driver.findElement(By.cssSelector(PropertiesRepository.getString("new.atitesting.cardback.dd.loc")));
			dropdown.click();
			...
			logger.info("Unable to clickOnHAModuleButton");
			log(logger, "Unable to clickOnHAModuleButton");
			throw new DriverException("Unable to verifyDropdownOptionInHACardback", e);
		}
		logger.info("Method: verifyDropdownOptionInHACardback End flag --> " + flag);
		return flag;
		
	}
	public boolean clickOnHAModuleButton(String assessmentName) throws Exception {
		...
		try {
			...
			logger.info("Unable to clickOnHAModuleButton");
			log(logger, "Unable to clickOnHAModuleButton");
			throw new DriverException("Unable to clickOnHAModuleButton", e);
		}
		logger.info("Method: clickOnHAModuleButton End flag --> " + flag);
		return flag;
	}
