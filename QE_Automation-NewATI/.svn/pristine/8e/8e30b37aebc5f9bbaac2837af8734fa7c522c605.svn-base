...
public class PaymentTest15 extends BaseTest {
	...
	@Test(priority = 1, description = "Regression  NewATI-338:Verify Self Pay Tutorial payment using a Discover card with shipping tax")
	public void NewATI338_verifySelfPayTutorialDiscoverCardCardWithTax() throws Exception {
		...
		paymentsPage.enterSelfPayProductID(
				PropertiesRepository.getString("new-atitesting.selfpay.tutorial.amex.with.tax.id.value"));
		...
		paymentsPage.enterSelfPayProductPassword(
				PropertiesRepository.getString("new-atitesting.selfpay.tutorial.amex.with.tax.pwd.value"));
		paymentsPage.clickOnAddProductContinueButton(false);
	...
	@TestAnnotations(testID = "NewATI-304")
	...
	@TestAnnotations(testID = "NewATI-310")
	@Test(priority = 3, alwaysRun = true, enabled = true, description = "NewATI-310:Verify (TEAS) Self Pay Assessment Payment using a Discover Card with shipping tax")
