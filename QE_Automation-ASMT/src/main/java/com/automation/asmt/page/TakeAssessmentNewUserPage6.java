...
public class TakeAssessmentNewUserPage6 extends QuestionTypePage {

	private Logger logger = LogManager.getLogger(this.getClass());
	
	public TakeAssessmentNewUserPage6(WebDriver webDriver) {
		super(webDriver);
	}
	 ...
	 * @throws Exception
	 */
	public boolean takeTestNewUser(boolean bool, boolean breakFlag,
		...
		try {
			...
			for (int i = 0; i <= count; i++) {
				...
				if (i == count) {
					...
					flag = verifyAssessment(rationaleFlag, flagquestion, "Finalize and View Results");
				} else {
					if (i == 0 && flagquestion) {
						switchToAssessmentFrameHandler();
						verifyFlag(true);
	 ...
	 * @throws Exception
	 */
	public boolean takeTestVerifyTimeForCABAssessment(boolean bool, boolean breakFlag,
			boolean rationaleFlag, boolean flagquestion) throws Exception {
		logger.info("Method : takeTestVerifyTimeForCABAssessment :::::::: Starts");
		boolean verifyTimer = false;
		boolean finalTimerFlag = true;
		...
		try {
			...
			for (int i = 0; i <= count; i++) {
				verifyTimer = false;
				verifyTimer = verifyElement(PropertiesRepository.getString("asmt.myati.assessment.timer.loc"));
				logger.info("Verifying Time for Question no --> " + i + "  value   " + verifyTimer);
				List<WebElement> checkBoxElement = baseHandler.findElements(PropertiesRepository.getString("asmt.generic.tutorial.checkbox.new.multiple.answer.loc"));
				List<WebElement> radioElement = baseHandler.findElements(PropertiesRepository.getString("asmt.generic.tutorial.radio.button.new.multiple.answer.loc"));
				...
				if(!verifyTimer) {
					logger.info("insider finalTimerflag section - Verifying Time for Question no --> " + i + "  value   " + verifyTimer);
					finalTimerFlag = false;
				}
				if (i == count) {
					switchToAssessmentFrameHandler();
					List<WebElement> buttonElement = baseHandler.findElements(PropertiesRepository.getString("new-atitesting.test.assessment.continue.button.loc"));
					...
					logger.info("Timer Flag  value for all questions  " + finalTimerFlag );
					logger.info("Completion section Flag   " + lastSection );
					if(finalTimerFlag && lastSection) {
			...
			logger.info("unable to take practice assessment in takeTestVerifyTimeForCABAssessment" + e);
		}
		waitUntillBufferInvisible(PropertiesRepository
				.getString("asmt.pleasewait.bufferLoc"));
		logger.info("Method : takeTestVerifyTimeForCABAssessment :::::::: END flag --> " + flag);
		return flag;
	}
	 ...
	 * @throws Exception
	 */
	public int takeTestReturnQuestionNo(boolean bool, boolean breakFlag,
			boolean rationaleFlag, boolean flagquestion) throws Exception {
		logger.info("Method : takeTest :::::::: Starts");
		int questionNo = 0;
		try {
		...
		waitUntillBufferInvisible(PropertiesRepository
				.getString("asmt.pleasewait.bufferLoc"));
		logger.info("Method : takeTest :::::::: END question no --> " + questionNo);
		return questionNo;
	}
}
