...
public class TEASProctorTest extends BaseTest{
	 ...
	 * @author syed.pasha
	 */
	@TestAnnotations(testID="ASMT-362_2")
	@Test(priority = 1, alwaysRun = true, groups = { "Smoke" }, description = "ASMT-362_2:Verify presence of all element and 'NEXT' button functionality in Proctored assessment page.")
	public void ASMT362_2_TEASProctorAss() throws Exception {
		logger.info("ASMT362_2_TEASProctorAss Start");
		
		
		userName = userCreationUtils.getUserDetails();
		logger.info("Student User Created  --> " + userName);
		String procAssessmentId = PropertiesRepository.getString("asmt.proctored.assessment.TEAS.id.value");
		String procAssessmentPwd = PropertiesRepository.getString("asmt.proctored.assessment.TEAS.pwd.value");
		...
		// Assessment Page.
		log(logger, "Taking TEAS Proctor Assessment Section -- 1");
		VerificationHandler.verifyTrue(maintenanceScriptPage.takeTestTEAS(false, false, false, false));
		
		log(logger, "Taking TEAS Proctor Assessment Section -- 2");
		VerificationHandler.verifyTrue(maintenanceScriptPage.takeTestTEAS(false, false, false, false));

		log(logger, "Taking TEAS Proctor Assessment Section -- 3");
		VerificationHandler.verifyTrue(maintenanceScriptPage.takeTestTEAS(false, false, false, false));

		log(logger, "Taking TEAS Proctor Assessment Section -- 4");
		VerificationHandler.verifyTrue(maintenanceScriptPage.takeTestTEAS(false, false, false, false));
		...
		logger.info("ASMT362_2_TEASProctorAss End");		
}
