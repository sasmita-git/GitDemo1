...
public class ExcelHandler {
	...
	/**
	 * Get a specific column from the Excel Worksheet (The first column is column 1)
	 *
	 * @param columnNumber
	 * @return
	 * @throws Exception
	...
	public int getColumnCount() throws Exception {
		return this.selectedSheet.getColumns();
	}
	
	public int getRowCount() throws Exception {
		return this.selectedSheet.getRows();
	} 
	
	/**
	 * Get a specific column from the Excel Worksheet You can optionally skip the top row of the
	 * column to ensure column titles are not pulled into the data set (The first column is column
	 * 1)
	 *
	 * @param columnNumber
	 * @param skipFirstRow
	 ...
	 * @throws Exception
	 */
	public HashMap<Integer, Cell> getColumn(int columnNumber, boolean skipFirstRow)
			throws Exception {
		if (this.selectedSheet.equals(null)) {
			...
			throw new Exception("There are only " + this.selectedSheet.getColumns()
	...
	/**
	 * Get a specific row from the Excel Worksheet You can optionally skip the first column of the
	 * row to ensure row titles are not pulled into the data set (The first row is row 1)
	 *
	 * @param rowNumber
	...
	public HashMap<Integer, Cell> getRow(int rowNumber, boolean skipFirstColumn) throws Exception {
		if (this.selectedSheet.equals(null)) {
			...
			throw new Exception("There are only " + this.selectedSheet.getRows()
	...
	/**
	 * This will map two rows into a HashMap. The key row will be converted into a string that can
	 * be used to reference the matching data in he value row. You can optionally skip the first
	 * column of the row to ensure row titles are not pulled into the data set (The first row is row
	 * 1)
	 */
	 ...
	 * @throws Exception
	 */
	public HashMap<String, Cell> mapTwoRows(int keyRow, int valueRow, boolean skipFirstColumn)
			throws Exception {
		if (this.selectedSheet.equals(null)) {
			throw new Exception(
					"No sheet selected.  You must select a sheet before trying to get data!");
		} else if ((keyRow > this.selectedSheet.getRows())
				|| (valueRow > this.selectedSheet.getRows())) {
			throw new Exception("There are only " + this.selectedSheet.getRows()
	...
	/**
	 * This will map two columns into a HashMap. The key column will be converted into a string that
	 * can be used to reference the matching data in he value column. You can optionally skip the
	 * first row of the column to ensure column titles are not pulled into the data set (The first
	 * row is row 1)
	 */
	...
	public HashMap<String, Cell> mapTwoColumns(int keyColumn, int valueColumn,
		...
		if (this.selectedSheet.equals(null)) {
			throw new Exception(
					"No sheet selected.  You must select a sheet before trying to get data!");
		} else if ((keyColumn > this.selectedSheet.getRows())
				|| (valueColumn > this.selectedSheet.getRows())) {
			throw new Exception("There are only " + this.selectedSheet.getRows()
					+ " columnss in this sheet.  Unable to select columns " + keyColumn + " and "
					+ valueColumn + "!");
		}
	...
	/**
	 * Get a specific cell from the Excel Worksheet (The top left cell is assumed to be in position
	 * 1, 1)
	 *
	 * @param column
	 * @param row
