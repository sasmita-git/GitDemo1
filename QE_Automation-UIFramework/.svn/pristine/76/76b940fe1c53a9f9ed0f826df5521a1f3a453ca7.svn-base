package com.ascendlearning.automation.ui.handlers.graphs;
...
abstract class HighCharts {
	...
	public String getToolTipLine(int lineNo) throws NoSuchElementException {
		...
		List<WebElement> toolTipLines = toolTip.findElements(By
				.cssSelector("text tspan"));
		for (WebElement toolTipLine : toolTipLines) {
			lines.add(toolTipLine.getText());
		}
		if (lineNo > lines.size()) {
			throw new NoSuchElementException("There is no line " + lineNo
					+ "! There are only " + lines.size()
					+ " lines in the tool tip");
		}
	...
	public List<String> getXAxisLabelsText() {
		...
		List<WebElement> xAxisLabels = getXAxisLabels().findElements(
				By.cssSelector("text"));
	...
	public List<String> getYAxisLabelsText() {
		List<String> labels = new ArrayList<String>();
		List<WebElement> yAxisLabels = getYAxisLabels().findElements(
				By.cssSelector("text"));
	...
	protected void hoverOverColumnOrBarChartSeriesAtXAxisPosition(int series,
			String xAxisLabel) {
		int barNumber = getXAxisLabelsText().indexOf(xAxisLabel);
/*		WebElement pointToHoverOver = chart.findElements(
				By.cssSelector("g.highcharts-tracker > g:nth-of-type(" + series
						+ ") > rect")).get(barNumber);

		// For browsers not supporting native events
		javascript.callEmbeddedSelenium(driver, "triggerEvent",
				pointToHoverOver, "mouseover");
